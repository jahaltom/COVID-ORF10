...
rule quant:
        ...
        run:
            # get sample name 
            id=str(output.quant_file).split("/")[1]
            #fastq file
            fq=id+".fq"                
        		#Pathway for fastq file and salmon output 
        		path=DIR + "/" + id+ "/"+
        		#Run Salmon on sra object(fastq file) and delete fastq when finished.  
        		sra.SRA(fastq=fq,directory=path).quant(salmon).delete_fastq()
...
rule merge:
	...
	run:
        ...
        gene_tpm=md.merge(df_gene_tpm, on=['Gene_stable_ID'], how='right')        	    	
        gene_count=md.merge(df_gene_count, on=['Gene_stable_ID'], how='right')

		#reorder so gene name is first.	
        gene_tpm = df_gene_tpm[ ['Gene_name'] + [ col for col in df_gene_tpm.columns if col != 'Gene_name' ] ]
        gene_count = df_gene_count[ ['Gene_name'] + [ col for col in df_gene_count.columns if col != 'Gene_name' ] ]
		
		
		
		df_gene_tpm.to_csv(DIR+'/results_TPM_gene.tsv',sep='\t',index=False)        
        gene_count.to_csv(DIR+'/results_Count_gene.tsv',sep='\t',index=False)
